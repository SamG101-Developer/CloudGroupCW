...
@function.route(route="roomAdultOnlySet", auth_level=func.AuthLevel.ANONYMOUS, methods=['POST'])
def roomAdultOnlySet(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        adultOnlyNewValue = reqJson['valueToSet']
        ...
        if adultOnlyNewValue != True and adultOnlyNewValue != False:
            return func.HttpResponse(body=json.dumps({'result': False,'msg':'valueToSet is not a boolean value'}))
        ...
        item_to_modify['adult_only'] = adultOnlyNewValue
        roomContainer.replace_item(item=item_to_modify, body=item_to_modify)

        return func.HttpResponse(body=json.dumps({'result': True, "msg": "Successfully changed adult only flag to: {}".format(adultOnlyNewValue)}), mimetype="application/json")
